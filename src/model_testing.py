...
checkpoint_path = "checkpoints/epochs_3/checkpoint-206013"  # Updated to epochs_3
...
def evaluate_model(model, tokenizer, test_dataset, batch_size=32, max_prompt_tokens=512):
    ...
    # Generate predictions in large GPU batches
    print("Generating predictions in parallel on GPU...")
    predictions = []
    total_batches = (len(queries) + batch_size - 1) // batch_size
    progress = tqdm(range(0, len(queries), batch_size), desc="Inference", total=total_batches)
    
    for start in progress:
        batch_prompts = queries[start:start + batch_size]
        tokenized = tokenizer(
            batch_prompts,
            return_tensors='pt',
            padding=True,
        ...
        generated_only = outputs[:, prompt_token_count:]
        generated_texts = tokenizer.batch_decode(generated_only, skip_special_tokens=True)
        
        for text in generated_texts:
            predictions.append(extract_prediction(text))
...
def main():
    ...
    results_file = os.path.join(output_folder, "test_results.txt")
    ...
    detailed_file = os.path.join(output_folder, "detailed_predictions.txt")
